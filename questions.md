...
4. what is io.copy and why is it important
   It is a function to copy from `io.reader` to `io.writer`. This function handles streaming data efficiently
...
9. why buffer is used with flush?
  because the buffer still holding the data in memory. to send it out to the next process, do the flush
10. what is io.pipe
...
io.Copy(out, r.Body)
```
  `io.Copy` is a function to copy bytes to `writer`. In this case, it copy to a file, means it saving it to local. This approach doesn't use memory because the `io.copy` itself is a stream function.
...
16. What is `stream` in go?
  Anything that implements `io.Reader` or `io.Writer` 
  E.g: 
  - HTTP body → file
  - file → HTTP response
  - TCP conn → TCP conn
  - Reader → gzip compressor → Writer
  - file → tar archive → network socket

17. What is bufio and when to use it?
  Bufio is a golang package for reading and writing to source efficiently due to the buffer usage. Instead of direct write/read that can increase usage of `syscall`, it stores the data first and send the chunked data at a time.
  Therefore it can reduce the amount of `syscall`. That is why bufio need to be flushed. These package is mainly useful when dealing with I/O.
  Use bufio when:
  - performing small reads or write to a file, socket, or other I/O stream.
  - want to improve the I/O efficieny by reducing system calls.
  - need helper functions like `readString()`, `ReadLine()`, or `Scanner`.
